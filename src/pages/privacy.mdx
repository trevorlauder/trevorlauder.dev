...
- Cloudflare Web Analytics is only injected for non-EU users by Cloudflare's edge logic, and does not use any client-side state, such as cookies or localStorage, to collect usage metrics. They also don't "fingerprint" individuals via their IP address, User Agent string, or any other data for the purpose of displaying analytics.
